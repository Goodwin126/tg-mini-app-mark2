<template>
  <div class="place-content-center mt-10">
    <div class="flex flex-col">
      <div class="flex grid grid-cols-3">
        <img class="flex-justify-left m-auto" src="/Ellipse 3.png" />
        <div class="flex flex-col m-auto mt-2.5 ml-0">
          <p class="text-lg text-white overflow-auto touch-none select-none">Вода 1,5 л</p>
          <img class="m-auto ml-0" src="/Runner_4.png" />
          <p class="text-xs text-gray-400 overflow-auto touch-none select-none">
            рекомендовано 2,3 л
          </p>
        </div>
        <img
          class="m-auto hover:cursor-pointer transition transform hover:scale-105 isLarge:scale-90"
          src="/Button.png"
        />
        <img class="flex-justify-left m-auto" src="/Ellipse 4.png" />
        <div class="flex flex-col mt-2.5">
          <p class="text-lg text-white overflow-auto touch-none select-none">Завтрак</p>
          <p class="text-xs text-gray-400 overflow-auto touch-none select-none">
            рекомендовано 410 ккал
          </p>
        </div>
        <img
          class="m-auto hover:cursor-pointer transition transform hover:scale-105 isLarge:scale-90"
          src="/Button.png"
        />
        <img class="flex-justify-left m-auto" src="/Ellipse 2.png" />
        <div class="flex flex-col mt-2.5">
          <p class="text-lg text-white overflow-auto touch-none select-none">Обед</p>
          <p class="text-xs text-gray-400 overflow-auto touch-none select-none">
            рекомендовано 540 ккал
          </p>
        </div>
        <img
          class="m-auto hover:cursor-pointer transition transform hover:scale-105"
          src="/Button.png"
        />
        <img class="flex-justify-left m-auto" src="/Ellipse 5.png" />
        <div class="flex flex-col mt-2.5">
          <p class="text-lg text-white overflow-auto touch-none select-none">Ужин</p>
          <p class="text-xs text-gray-400 overflow-auto touch-none select-none">
            рекомендовано 320 ккал
          </p>
        </div>
        <img
          class="m-auto hover:cursor-pointer transition transform hover:scale-105"
          src="/Button.png"
        />
      </div>
    </div>
  </div>
  <div class="flex grid grid-cols-4 mt-6">
    <img
      class="m-auto w-14 h-14 focus:outline-none transition transform hover:scale-105 hover:-translate-y-1"
      src="/diary.png"
    />
    <img
      class="m-auto w-14 h-14 focus:outline-none transition transform hover:scale-105 hover:-translate-y-1"
      src="/study.png"
    />
    <img
      status
      class="m-auto w-14 h-14 focus:outline-none transition transform hover:scale-105 hover:-translate-y-1"
      src="/awards.png"
    />
    <img
      class="m-auto w-14 h-14 focus:outline-none transition transform hover:scale-105 hover:-translate-y-1"
      src="/results.png"
    />
    <p
      class="m-auto text-base font-bold text-black hover:text-blue-800 overflow-auto touch-none select-none"
    >
      Дневник
    </p>
    <p
      class="m-auto text-base font-bold text-black hover:text-blue-800 overflow-auto touch-none select-none"
    >
      Изучить
    </p>
    <p
      class="m-auto text-base font-bold text-black hover:text-blue-800 overflow-auto touch-none select-none"
    >
      Награды
    </p>
    <p
      class="m-auto text-base font-bold text-black hover:text-blue-800 overflow-auto touch-none select-none"
    >
      Результаты
    </p>
  </div>
</template>
